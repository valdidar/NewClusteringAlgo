...
my_cluster <- function(data, num_clusters) {
  ...
  plot <- ggplot(df, aes(x = x, y = y, color = cluster)) +
    geom_point() +
    theme_minimal()
  ...
  for (i in 1:num_clusters) {
    ...
    centroid <- apply(hull_points[, c("x", "y")], 2, mean)
    angles <- atan2(hull_points$y - centroid[2], hull_points$x - centroid[1])
    angles_df <- data.frame(x = hull_points$x, y = hull_points$y, angle = angles)
...
# apply the custom clustering algorithm to the sample data
